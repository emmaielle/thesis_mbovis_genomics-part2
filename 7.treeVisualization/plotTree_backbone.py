...
def parse_args():
	...
	parser.add_argument('--delete_branches', nargs='+', type=str, required=False, help='Name/s of the branches not to show in the tree')
...
def getColourPalette(values, args, this_label):
	...
	simple_palette = ['White', '#78c679', '#006837', '#df65b0', '#cc4c02', '#fe9929', '#c2e699', '#ce1256', '#31a354']
...
########### COMENTADO PARA VER COMO QUEDA CON EL BARCODE. PORQUE ESE NO TIENE TODOS LOS NODOS
	...
	node = matches[0]
	node.swap_children()
	matches = search_by_size(tree, 16)
	print("matches")

	node = matches[0]
	node.swap_children()

	matches = search_by_size(tree, 17)
	print("matches")

	node = matches[0]
	node.swap_children()

	matches = search_by_size(tree, 21)
	print("matches")
